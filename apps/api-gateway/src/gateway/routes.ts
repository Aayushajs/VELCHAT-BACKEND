...
/** Base URL for a route's upstream — env override (`UPSTREAM_<SERVICE>`) or the dev localhost port. */
export function upstreamFor(route: Route): string {
  return process.env[`UPSTREAM_${route.service}`] || `http://localhost:${route.port}`;
}
