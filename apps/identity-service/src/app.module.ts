...
import { composeSingle, identityGroup, type ServiceDeps } from '@velchat/composition';

export type AppDeps = ServiceDeps;
export { INFRA } from '@velchat/composition';

/**
 * identity-service — one feature group in one process.
 *
...
export class AppModule {
  static forRoot(deps: AppDeps): DynamicModule {
    return composeSingle(AppModule, deps, identityGroup(deps.config, deps.logger));
  }
}
